...
class ApplicationsController extends Controller
{
    ...
    /**
     * Display a listing of the resource (ALL applications, without pagination).
     */
    public function index(Request $request)
    {
        $user = $request->user();
        ...
        // Hamma arizalarni olish (paginatsiyasiz)
        ...
        // Har bir ariza uchun malumotlarni formatlash
        $data = $applications->map(function (Application $app) {
            ...
            $vacancyData = [
                ...
                'company'      => $raw['employer']['name'] ?? null,
                'title'        => $vacancy?->title,
                'location'     => $vacancy?->area?->name ?? ($raw['area']['name'] ?? null),
                'experience'   => $raw['experience']['name'] ?? null,
                'salary'       => $raw['salary'] ?? null,
                'published_at' => isset($raw['published_at'])
                    ? Carbon::parse($raw['published_at'])->format('Y-m-d H:i:s')
            ...
            return [
                ...
                'vacancy_id'    => $app->vacancy_id,
                'score_percent' => (int) round((float) ($app->match_score ?? 0)),
                'status'        => true, // all applications = applied
                'vacancy'       => $vacancyData,
            ];
        });
