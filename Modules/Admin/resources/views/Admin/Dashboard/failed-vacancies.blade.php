...
@section('content')
    @php
        ...
        $totalFailed = $vacancies instanceof \Illuminate\Contracts\Pagination\LengthAwarePaginator ? $vacancies->total() : $collection->count();
    @endphp
    ...
    <div class="page-header">
        <div class="page-header-left d-flex align-items-center">
            ...
            <ul class="breadcrumb">
                ...
                <li class="breadcrumb-item">Failed</li>
            </ul>
        </div>
    </div>
        ...
        <div class="card-header d-flex align-items-center justify-content-between">
            <div class="filters">
                <div class="btn-group" role="group">
                    <a href="{{ route('admin.vacancies.failed', ['filter' => 'all']) }}" class="btn btn-sm btn-outline-primary {{ $currentFilter==='all' ? 'active' : '' }}">All</a>
                    <a href="{{ route('admin.vacancies.failed', ['filter' => 'telegram']) }}" class="btn btn-sm btn-outline-primary {{ $currentFilter==='telegram' ? 'active' : '' }}">Telegram</a>
                    <a href="{{ route('admin.vacancies.failed', ['filter' => 'hh']) }}" class="btn btn-sm btn-outline-primary {{ $currentFilter==='hh' ? 'active' : '' }}">HH</a>
                </div>
            </div>
        </div>
        <div class="table-responsive">
            <table class="table align-middle mb-0">
                ...
                <tbody>
                    ...
                    @forelse($vacancies as $index => $vacancy)
                        ...
                        <tr class="categories-list-row" onclick="window.location.href='{{ route('admin.vacancies.show', $viewParams) }}'">
                            ...
                            <td class="text-end" data-label="Actions">
                                <form method="POST" action="{{ route('admin.vacancies.requeue', ['vacancy' => $vacancy->id]) }}" onClick="event.stopPropagation();">
                                    @csrf
                                    <button type="submit" class="btn btn-sm btn-warning">
                                        <i class="feather-rotate-ccw"></i> Re‑queue
                                    </button>
                                </form>
                            </td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="4" class="text-center py-4 text-muted">No failed vacancies.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
