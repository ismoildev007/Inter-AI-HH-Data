...
@section('content')
                        ...
                        <tr onclick="window.location.href='{{ route('admin.resumes.show', $r->id) }}'">
                            ...
                            <td class="text-end text-nowrap" data-label="Actions">
                                <button type="button"
                                        ...
                                        data-bs-target="#resumeCategoryModal"
                                        data-resume-id="{{ $r->id }}"
                                        data-resume-title="{{ $r->title ?? '' }}"
                                        data-resume-category="{{ $r->category ?? '' }}"
    ...
    <div class="modal fade" id="resumeCategoryModal" tabindex="-1" aria-labelledby="resumeCategoryModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                <form method="POST" data-action-template="{{ route('admin.resumes.update_category', ['resume' => '__RESUME__']) }}">
                    @csrf
                    @method('PATCH')
                    <div class="modal-header">
                        <h5 class="modal-title" id="resumeCategoryModalLabel">Update resume category</h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <div class="modal-body">
                        <p class="mb-3 text-muted small">Editing: <span class="fw-semibold" data-resume-title></span></p>
                        <div class="mb-3">
...
document.addEventListener('DOMContentLoaded', function () {
    const modalEl = document.getElementById('resumeCategoryModal');
    if (!modalEl) return;
    ...
    const titleHolder = modalEl.querySelector('[data-resume-title]');
    const categorySelect = modalEl.querySelector('select[name="category"]');

    modalEl.addEventListener('show.bs.modal', function (event) {
        const trigger = event.relatedTarget;
        if (!trigger) return;
        const resumeId = trigger.getAttribute('data-resume-id') || '';
        const resumeTitle = trigger.getAttribute('data-resume-title') || '';
        const currentCategory = trigger.getAttribute('data-resume-category') || '';

        if (form && actionTemplate) {
            form.action = resumeId ? actionTemplate.replace('__RESUME__', resumeId) : '#';
        }
        if (titleHolder) {
            titleHolder.textContent = resumeTitle;
        }
        if (categorySelect) {
